...
export const uploadEpisodeMeta = createAsyncThunk('episodes/uploadEpisodeMeta', async (formData, { rejectWithValue }) => {
    try {
...
// Admin: Create anime
export const createAnime = createAsyncThunk('episodes/createAnime', async (data, { rejectWithValue }) => {
    try {
...
// Admin: Update anime
export const updateAnime = createAsyncThunk('episodes/updateAnime', async ({ id, data }, { rejectWithValue }) => {
    try {
...
// Admin: Delete anime
export const deleteAnime = createAsyncThunk('episodes/deleteAnime', async (id, { rejectWithValue }) => {
    try {
...
const episodeSlice = createSlice({
    ...
    extraReducers: (builder) => {
        ...
        builder.addMatcher(
            (action) => action.type.startsWith('episodes/') && action.type.endsWith('/pending'),
            (state) => {
                state.isLoading = true;
                state.error = null;
                state.successMessage = null;
        ...
        builder.addMatcher(
            (action) => action.type.startsWith('episodes/') && action.type.endsWith('/fulfilled'),
            (state) => {
                state.isLoading = false;
        ...
        builder.addMatcher(
            (action) => action.type.startsWith('episodes/') && action.type.endsWith('/rejected'),
            (state, action) => {
                state.isLoading = false;
                state.error = action.payload;
            }

...
export default function Search() {
  ...
  const { animeList, pagination, isLoading } = useSelector((state) => state.anime);
  ...
  }, [dispatch, sortParam, queryParam, surpriseParam, genresParam, yearParam, statusParam, pageParam]);

  useEffect(() => {
    // If surprise me is active and data is loaded, pick one randomly and redirect!
    if (surpriseParam && !isLoading && animeList?.length > 0 && !surpriseHandled) {
        ...
        <main className="flex-1 w-full flex flex-col gap-8">
          ...
          {isLoading ? (
            ...
            <>
              ...
              <div className="flex justify-center items-center gap-2 mt-8">
                ...
                <span className="text-sm text-neutral-400 font-bold px-4">
                  Page {pageParam || '1'} {pagination?.total ? `of ${Math.ceil(pagination.total / (pagination.limit || 30))}` : ''}
                </span>
                
                <button 
                  disabled={animeList.length < 30 || (pagination && pagination.page >= Math.ceil(pagination.total / pagination.limit))}
                  onClick={() => {
                    const newParams = new URLSearchParams(location.search);
                    const currentPage = parseInt(pageParam) || 1;
                    newParams.set('page', currentPage + 1);
            ...
            <div className="w-full py-20 flex flex-col items-center justify-center text-center gap-4 bg-[#110e16] rounded-2xl border border-white/5 shadow-xl">
              ...
              <h2 className="text-xl font-bold text-neutral-400 uppercase tracking-widest">No anime found</h2>
              <p className="text-neutral-600 text-sm">Try adjusting your filters or search query.</p>
            </div>
          )}
        </main>

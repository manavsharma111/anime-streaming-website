...
import { getWatchHistory, deleteAllHistory, deleteHistory } from '../redux/slice/historySlice';
import { User, Settings, Clock, Trash2, Edit2, Play, LogOut, Loader } from 'lucide-react';
...
export default function Profile() {
  ...
  const { history, isLoading: historyLoading } = useSelector(state => state.history);
      ...
      <div className="max-w-[1200px] mx-auto flex flex-col md:flex-row gap-8">
        ...
        <div className="w-full md:w-[300px] flex-shrink-0 flex flex-col gap-6">
          ...
          <div className="bg-[#110e16] rounded-2xl border border-white/5 overflow-hidden flex flex-col">
            ...
            >
              <Clock size={18} /> <span className="font-bold text-sm">Watch History</span>
            </button>
            <button 
            ...
            >
              <Settings size={18} /> <span className="font-bold text-sm">Account Settings</span>
            </button>
          </div>
        </div>
        ...
        <div className="flex-1 min-w-0">
          ...
          {activeTab === 'history' && (
            <div className="bg-[#110e16] p-6 sm:p-8 rounded-2xl border border-white/5">
              <div className="flex items-center justify-between mb-8 pb-4 border-b border-white/5">
                <h3 className="text-2xl font-black uppercase tracking-wider">Watch History</h3>
                {history?.length > 0 && (
                  <button 
              ...
              {historyLoading ? (
                <div className="flex justify-center py-12"><Loader size={32} className="animate-spin text-[#f33767]" /></div>
              ) : history?.length > 0 ? (
                <div className="flex flex-col gap-4">
                  {history.map((item, index) => {
                    ...
                    return (
                      <div key={`${item._id}-${index}`} className="flex gap-4 p-3 rounded-xl bg-white/5 hover:bg-white/10 transition-colors border border-white/5 group relative overflow-hidden">
                        <Link 
                        ...
                        >
                          ...
                          {progressPercent > 0 && (
                            <div className="absolute bottom-0 left-0 w-full h-1.5 bg-black/60">
                              <div className="h-full bg-[#f33767]" style={{ width: `${Math.min(100, Math.max(0, progressPercent))}%` }}></div>
                            </div>
                          )}
                        </Link>
                        
                        <div className="flex flex-col justify-center flex-1 py-1">
                          <Link to={`/watch/${item.episode._id || item.episode}`}>
                            <h4 className="text-base font-bold text-white line-clamp-1 hover:text-[#f33767] transition-colors">{item.anime.title}</h4>
                          </Link>
                          <p className="text-xs text-neutral-400 mt-1">Episode {item.episode.episodeNumber}</p>
                          {item.completed && <span className="text-[10px] font-bold uppercase tracking-widest text-green-500 mt-2 bg-green-500/10 px-2 py-1 w-fit rounded">Completed</span>}
                        </div>
                ...
                <div className="text-center py-16">
                  ...
                  <p className="text-neutral-500 text-sm">You haven't watched any anime recently.</p>
                </div>
              )}
            </div>
          )}
          ...
          {activeTab === 'settings' && (
            <div className="bg-[#110e16] p-6 sm:p-8 rounded-2xl border border-white/5">
              <div className="flex items-center justify-between mb-8 pb-4 border-b border-white/5">
                <h3 className="text-2xl font-black uppercase tracking-wider">Account Settings</h3>
                {!isEditing && (
                  <button 
              ...
              <form onSubmit={handleUpdateProfile} className="max-w-md flex flex-col gap-6">
                ...
                <div className="flex flex-col gap-2">
                  <label className="text-xs font-bold uppercase tracking-widest text-neutral-400">Email <span className="text-neutral-600 lowercase">(cannot be changed)</span></label>
                  <input 
                    type="email" 
                    value={user?.email || ''}
                    disabled
                ...
                <div className="flex flex-col gap-2">
                  <label className="text-xs font-bold uppercase tracking-widest text-neutral-400">Avatar URL</label>
                  <input 
                    type="text" 

      ...
      <div className="my-8 px-4 md:px-12">
        ...
        <div className="flex gap-4 overflow-hidden">
          {[...Array(6)].map((_, i) => (
            <div
              key={i}
              className="min-w-[160px] md:min-w-[240px] aspect-[16/9] bg-neutral-900 animate-pulse rounded-md flex-shrink-0"
            />
          ))}
        </div>
      </div>
      ...
      >
        {animes.map((anime) => (
          ...
          >
            <Link
              to={`/anime/${anime._id}`}
              className="block relative aspect-[16/9] rounded-md overflow-hidden bg-neutral-900 border border-white/5 shadow-lg"
            >
              <img
              ...
              <div className="absolute top-2 left-2 flex gap-1 z-10">
                <span className="bg-red-600 text-[10px] font-black px-1.5 py-0.5 rounded shadow-lg">
                  EP {anime.episodes?.length || 0}
                </span>
              </div>
              ...
              <AnimatePresence>
                {hoveredAnime === anime._id && (
                  ...
                  >
                    ...
                    <h3 className="text-white font-black text-sm line-clamp-1 mb-1 drop-shadow-md">
                      {anime.title}
                    </h3>

                    <div className="flex items-center gap-2 text-[10px] font-bold text-neutral-300 mb-1">
                      <span className="text-green-500">
                        {anime.rating * 10 || 85}% Match
                      </span>
                      <span className="border border-neutral-500 px-1 rounded">
                        {anime.ageRating || "16+"}
                      </span>
                    ...
                    <div className="flex flex-wrap items-center gap-1.5 text-[10px] text-neutral-400">
                      {(anime.genres || []).slice(0, 3).map((g, idx) => (
                        <span key={idx} className="flex items-center">
                          {g}
                          {idx < Math.min(anime.genres.length - 1, 2) && (

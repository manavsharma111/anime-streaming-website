...
export default function QuickFilter() {
  ...
  const [availableGenres, setAvailableGenres] = useState(['Action', 'Adventure', 'Comedy', 'Drama', 'Fantasy', 'Romance', 'Sci-Fi']);
  ...
  const filterOptions = [
    ...
    { key: 'year', label: 'Year', options: ['2024', '2023', '2022', '2021', '2020'] },
    { key: 'status', label: 'Status', options: ['Ongoing', 'Completed'] },
    { key: 'sort', label: 'Order', options: [{name: 'Rating', val: 'rating'}, {name: 'Latest', val: 'latest'}, {name: 'Oldest', val: 'oldest'}] }
  ];
  ...
  const handleSearch = () => {
    ...
    if (filters.genres) params.set('genres', filters.genres); else params.delete('genres');
            ...
            <CustomSelect
              options={[
                ...
                ...filterGroup.options.map(opt => {
                  ...
                  return { label: isObj ? opt.name : opt, value: isObj ? opt.val : opt };
                })
              ]}

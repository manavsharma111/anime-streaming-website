      ...
      >
        <motion.div 
          ...
          animate={!isMobile ? {
            ...
            backgroundColor: isScrolled ? "rgba(17,14,22,0.6)" : "rgba(17,14,22,0.3)",
            borderColor: isScrolled ? "rgba(255,255,255,0.05)" : "transparent",
            borderWidth: isScrolled ? "1px" : "0px",
            boxShadow: isScrolled ? "0 10px 40px rgba(0,0,0,0.5), inset 0 0 0 1px rgba(255,255,255,0.05)" : "none",
          ...
          style={{ 
            ...
            justifyContent: isMobile ? "space-evenly" : (isScrolled ? "center" : "space-between")
          }}
        >
          ...
          >
              ...
              return (
                <button
                  ...
                  onMouseEnter={() => {
                    if (!isMobile) {
                      setActiveTab(item.id);
                      if (item.id === 'user') { setDropdownOpen(true); setNotifDropdownOpen(false); }
                      else if (item.id === 'notifications') { setNotifDropdownOpen(true); setDropdownOpen(false); }
                      else { setDropdownOpen(false); setNotifDropdownOpen(false); }
                    }
                  }}
            ...
            <AnimatePresence>
              {dropdownOpen && (
                ...
                >
                  <div className={!isMobile ? "bg-neutral-950/95 backdrop-blur-2xl border border-neutral-800 shadow-[0_15px_40px_rgba(0,0,0,0.8)] rounded-2xl overflow-hidden" : "h-full"}>
                    {isMobile && <div className="w-12 h-1 bg-neutral-800 rounded-full mx-auto mb-5" />}

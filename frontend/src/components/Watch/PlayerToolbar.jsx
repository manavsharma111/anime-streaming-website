...
import { Monitor, Settings, Scissors, SkipBack, SkipForward, Play, Download, Bookmark, Heart } from 'lucide-react';
...
export default function PlayerToolbar({ 
  ...
  handlePrev, handleNext, currentIndex, episodesList,
  episode, backendUrl
}) {
  ...
  const wishlistItem = wishlist?.find(item => 
    item.anime?._id === episode?.anime?._id || item.anime === episode?.anime?._id
  );
    ...
    <div className="flex flex-col bg-[#1c1c1c]">
      ...
      <div className="w-full px-4 py-3 flex flex-wrap items-center justify-between gap-4 text-xs font-bold text-neutral-400">
        ...
        <div className="flex items-center gap-4 flex-wrap">
          ...
          <label className="flex items-center gap-1.5 cursor-pointer hover:text-white transition-colors">
            <input type="checkbox" checked={autoPlay} onChange={() => setAutoPlay(!autoPlay)} className="accent-[#6c5ce7]" />
            Auto Play
          </label>

          <label className="flex items-center gap-1.5 cursor-pointer hover:text-white transition-colors">
            <input type="checkbox" checked={autoNext} onChange={() => setAutoNext(!autoNext)} className="accent-[#6c5ce7]" />
            Auto Next
          </label>

          <label className="flex items-center gap-1.5 cursor-pointer text-[#ffc107] hover:text-[#ffca28] transition-colors">
            <input type="checkbox" checked={autoSkip} onChange={() => setAutoSkip(!autoSkip)} className="accent-[#ffc107]" />
            Auto Skip
          </label>
          ...
          <button 
            onClick={handleNext}
            disabled={currentIndex < 0 || currentIndex >= episodesList.length - 1}
      ...
      <div className="w-full bg-[#111] p-4 flex flex-col md:flex-row items-center justify-between gap-4">
        <div className="text-xs text-neutral-400 leading-relaxed max-w-md">
          You're watching <span className="text-[#6c5ce7] font-bold">Episode {episode.episodeNumber}</span>. <br />
          ...
          <div className="flex items-center gap-2">
            <button 
              ...
              className={`px-4 py-1.5 rounded text-xs font-bold transition-all flex items-center gap-2 ${
                activeServer === 1 ? 'bg-[#6c5ce7] text-white' : 'bg-[#1c1c1c] text-neutral-400 hover:bg-white/10 hover:text-white'
              }`}
            >
              <span className={`w-2 h-2 rounded-full ${activeServer === 1 ? 'bg-white' : 'bg-neutral-600'}`}></span>
              Vidplay
            </button>
            <button 
              onClick={() => setActiveServer(2)} 
              className={`px-4 py-1.5 rounded text-xs font-bold transition-all flex items-center gap-2 ${
                activeServer === 2 ? 'bg-[#6c5ce7] text-white' : 'bg-[#1c1c1c] text-neutral-400 hover:bg-white/10 hover:text-white'
              }`}
            >
              <span className={`w-2 h-2 rounded-full ${activeServer === 2 ? 'bg-white' : 'bg-neutral-600'}`}></span>
              MyCloud
            </button>
          </div>
        </div>

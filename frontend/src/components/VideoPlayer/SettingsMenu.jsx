...
export default function SettingsMenu({
      ...
      {items.length > 0 ? (
        items.map((item) => {
          ...
          return (
            ...
            >
              <span className="w-1.5 text-center leading-none text-[16px]">
                {isActive ? "•" : ""}
              </span>
        ...
        <div className="text-left px-5 py-2 text-[13px] text-neutral-500">
          {emptyMessage}
        </div>
      )}
    </div>
  ...
  const displayDownloadQualities =
      ...
      <AnimatePresence>
        {showSettings && (
          <motion.div
            ...
            className="absolute bottom-full mb-2 right-0 sm:-right-4 md:right-0 w-[240px] sm:w-[280px] max-w-[90vw] max-h-[180px] sm:max-h-[60vh] md:max-h-[80vh] bg-[#1c1c1c]/95 backdrop-blur-md rounded-xl shadow-2xl z-50 flex flex-col border border-white/10 overflow-hidden origin-bottom-right"
            onWheel={(e) => e.stopPropagation()}
            onTouchMove={(e) => e.stopPropagation()}
          >
            {/* Header / Tabs */}

...
export default function SeekBar({ duration, currentTime, buffered, onSeek, className }) {
  ...
  return (
    <div 
      className={cn("w-full h-5 flex items-center group/seekbar cursor-pointer relative", className)}
    ...
    >
      ...
      <div ref={barRef} className="relative w-full h-1.5 bg-slate-600/50 rounded-full transition-all duration-200 group-hover/seekbar:h-2">
        {/* Buffered */}
        <div ref={bufferRef} className="absolute top-0 left-0 h-full bg-slate-400/40 rounded-full" style={{ width: '0%' }} />
        
        {/* Active Progress */}
        <div ref={progressRef} className="absolute top-0 left-0 h-full bg-blue-600 rounded-full flex items-center justify-end" style={{ width: '0%' }}>
          {/* Handle */}
          <div 
            ref={handleRef}
            className={cn(

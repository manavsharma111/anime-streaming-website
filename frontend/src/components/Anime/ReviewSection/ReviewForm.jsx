...
const ReviewForm = ({ animeId }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    const result = await dispatch(addReviewAsync({
      animeId,
  ...
  if (!user) {
    return (
      <div className="bg-[#110e16] border border-white/5 rounded-2xl p-6 text-center shadow-xl">
        <h3 className="text-white font-black uppercase tracking-widest mb-2">Write a Review</h3>
  ...
  return (
    ...
    >
      ...
      <h3 className="text-white font-black uppercase tracking-widest mb-4">Write a Review</h3>
      ...
      <form onSubmit={handleSubmit} className="space-y-4 relative z-10">
        <div>
          <label className="block text-xs text-neutral-500 font-bold uppercase tracking-widest mb-2">Your Rating</label>
          <StarRating rating={rating} setRating={setRating} size={28} />
        </div>

        <div>
          <label className="block text-xs text-neutral-500 font-bold uppercase tracking-widest mb-2">Your Review</label>
          <textarea
            value={comment}

...
export default function CustomSelect({ options, value, onChange, placeholder, className, containerClassName, searchable = false }) {
  ...
  const filteredOptions = searchable 
    ? options.filter(opt => (opt.label || '').toString().toLowerCase().includes(searchTerm.toLowerCase()))
    : options;
  ...
  return (
    <div className={`relative ${containerClassName || ''}`} ref={dropdownRef}>
      ...
      >
        ...
        <ChevronDown size={16} className={`transition-transform duration-300 text-neutral-500 ${isOpen ? 'rotate-180' : ''}`} />
      </button>
        ...
        {isOpen && (
          ...
          >
            ...
            {filteredOptions.length === 0 && (
              <div className="px-4 py-3 text-sm text-neutral-500 text-center">No options found</div>
            )}
          </motion.div>
        )}
      </AnimatePresence>

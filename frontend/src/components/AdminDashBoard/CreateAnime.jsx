...
import { Plus, Image as ImageIcon, Film, Calendar, Star, Info } from 'lucide-react';
...
export default function CreateAnime({ onSuccess }) {
  const [animeData, setAnimeData] = useState({ 
    ...
    cover: '', thumbnail: '', trailerUrl: '', year: new Date().getFullYear(), rating: 8.0 
  });
  ...
  const handleCreateAnimeSubmit = async (e) => {
    ...
    try {
      const payload = {
        ...animeData,
        genres: [...new Set(animeData.genres.split(',').map(g => g.trim()).filter(Boolean))]
      };
      await axiosInstance.post('/anime-admin/create', payload);
      setAnimeData({ title: '', description: '', genres: '', type: 'TV', cover: '', thumbnail: '', trailerUrl: '', year: new Date().getFullYear(), rating: 8.0 });
  ...
  return (
    ...
    >
      <div className="mb-8">
        ...
        <p className="text-neutral-400 text-sm">Add a new anime master record to the database before uploading episodes.</p>
      </div>
      ...
      <form onSubmit={handleCreateAnimeSubmit} className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="space-y-2">
            ...
            <input 
              type="url" value={animeData.cover} 
              onChange={e => setAnimeData({...animeData, cover: e.target.value})} 
              placeholder="https://..." 
              className="w-full bg-neutral-950/50 border border-neutral-800 rounded-xl px-4 py-3.5 text-sm focus:outline-none focus:border-red-500 text-white transition-colors" 
              required 
            />
          ...
          <div className="space-y-2">
            ...
            <input 
              type="url" value={animeData.thumbnail} 
              onChange={e => setAnimeData({...animeData, thumbnail: e.target.value})} 
              placeholder="https://..." 
              className="w-full bg-neutral-950/50 border border-neutral-800 rounded-xl px-4 py-3.5 text-sm focus:outline-none focus:border-red-500 text-white transition-colors" 
              required 
            />

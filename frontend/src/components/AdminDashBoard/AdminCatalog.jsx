...
import { Search, ChevronLeft, ChevronRight, Eye, ListVideo, Image as ImageIcon, Sparkles, Edit, Trash2, Film, X } from 'lucide-react';
...
export default function AdminCatalog() {
  ...
  const fetchCatalog = async (page = 1, searchQuery = "") => {
    ...
    try {
      const res = await axiosInstance.get(`/anime`, {
        params: { page, limit: pagination.limit, search: searchQuery, sort: 'latest' }
      });
      if (res.data.success) {
  ...
  if (selectedAnime) {
    return <AdminEpisodes anime={selectedAnime} onBack={() => { setSelectedAnime(null); fetchCatalog(pagination.page, search); }} />;
  }
  ...
  if (editingAnime) {
    return (
      <EditAnime 
        ...
        onSuccess={() => { setEditingAnime(null); fetchCatalog(pagination.page, search); }} 
      />
    );
  }
    ...
    >
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-6">
        <div>
          ...
          <p className="text-sm text-neutral-400 mt-2 tracking-wide">Showing page {pagination.page} of {totalPages} ({pagination.total} total entries)</p>
        </div>
      ...
      <div className="overflow-hidden rounded-2xl border border-white/5 bg-black/20">
        <div className="w-full">
          ...
          <div className="hidden md:grid grid-cols-12 gap-4 bg-neutral-900/50 border-b border-white/5 p-5">
            <div className="col-span-1 text-xs font-bold text-neutral-500 uppercase tracking-wider">Cover</div>
            <div className="col-span-4 text-xs font-bold text-neutral-500 uppercase tracking-wider">Title & Metadata</div>
            <div className="col-span-2 text-xs font-bold text-neutral-500 uppercase tracking-wider text-center">Status</div>
            <div className="col-span-2 text-xs font-bold text-neutral-500 uppercase tracking-wider text-right">Episodes</div>
            <div className="col-span-1 text-xs font-bold text-neutral-500 uppercase tracking-wider text-right">Views</div>
            <div className="col-span-2 text-xs font-bold text-neutral-500 uppercase tracking-wider text-center">Actions</div>
          </div>
          
          <div>
            <AnimatePresence mode="wait">
              {loading ? (
                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="p-12 text-center text-neutral-500">
                ...
                <motion.div key="catalog-list" className="divide-y divide-white/5" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                  ...
                  >
                    ...
                    <div className="md:col-span-1 flex-shrink-0">
                      {anime.thumbnail ? (
                        <div className="relative overflow-hidden rounded-lg shadow-lg w-full max-w-[120px] md:w-14 aspect-[2/3] mx-auto md:mx-0 bg-neutral-950">
                          <img src={getImageUrl(anime.thumbnail)} alt="cover" className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-500" />
                        </div>
                      ) : (
                    ...
                    <div className="md:col-span-4 text-center md:text-left w-full">
                      <p className="font-black text-white text-lg md:text-base mb-2 line-clamp-1 group-hover:text-indigo-400 transition-colors">{anime.title}</p>
                      <div className="flex flex-wrap items-center justify-center md:justify-start gap-2 text-[10px] font-bold text-neutral-500 uppercase tracking-wider">
                        <span className="bg-neutral-800/50 px-2 py-1 rounded-md text-neutral-300">{anime.year}</span>
                    ...
                    <div className="md:col-span-2 flex justify-center md:block w-full">
                      <span className={`inline-flex items-center px-3 py-1.5 md:px-2.5 md:py-1 rounded-md text-xs md:text-[10px] font-black uppercase tracking-widest ${anime.status === 'completed' ? 'bg-blue-500/10 text-blue-500 border border-blue-500/20' : 'bg-emerald-500/10 text-emerald-500 border border-emerald-500/20'}`}>
                        {anime.status}
                      </span>
                    </div>
                    ...
                    <div className="md:hidden flex justify-center gap-8 w-full border-y border-white/5 py-3 my-2">
                      <div className="flex flex-col items-center gap-1">
                        <span className="text-[10px] font-bold text-neutral-500 uppercase">Episodes</span>
                        <div className="flex items-center gap-1 text-white font-black"><ListVideo className="w-4 h-4 text-neutral-500" /> {anime.episodes?.length || 0}</div>
                      </div>
                      <div className="flex flex-col items-center gap-1">
                        <span className="text-[10px] font-bold text-neutral-500 uppercase">Views</span>
                        <div className="flex items-center gap-1 text-white font-black"><Eye className="w-4 h-4 text-neutral-500" /> {(anime.views || 0).toLocaleString()}</div>
                      </div>
                    </div>
                    ...
                    <div className="md:col-span-2 flex justify-center w-full mt-2 md:mt-0">
                      <div className="flex items-center gap-4 md:gap-3">
                        ...
                        <button 
                          onClick={(e) => { e.stopPropagation(); setEditingAnime(anime); }}
                          title="Edit Anime"
                        ...
                        <button 
                          onClick={(e) => { e.stopPropagation(); handleDeleteAnime(anime._id); }}
                          title="Delete Anime"

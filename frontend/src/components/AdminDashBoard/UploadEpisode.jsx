...
export default function UploadEpisodeForm({ animesList }) {
  ...
  const fetchSkipTimes = async () => {
    if (!formData.anime || !formData.episodeNumber) {
      return setStatusMessage({
        type: "error",
        text: "Select an Anime and enter an Episode Number first.",
      })
    }
    ...
    try {
      ...
      const jikanRes = await axios.get(
        `https://api.jikan.moe/v4/anime?q=${encodeURIComponent(selectedAnime.title)}&limit=1`,
      )
      if (!jikanRes.data.data || jikanRes.data.data.length === 0) {
        throw new Error(
          "Could not find this anime on MyAnimeList to fetch skip times.",
        )
      }
      const malId = jikanRes.data.data[0].mal_id

      const aniskipRes = await axios.get(
        `https://api.aniskip.com/v2/skip-times/${malId}/${formData.episodeNumber}?types[]=op&types[]=ed&episodeLength=0`,
      )
      ...
      if (aniskipRes.data.found) {
        ...
        setStatusMessage({
          type: "success",
          text: `Found ${op ? "Intro" : ""} ${op && ed ? "and" : ""} ${ed ? "Outro" : ""} skip times!`,
        })
      } else {
        setStatusMessage({
          type: "error",
          text: "No skip times available for this episode on AniSkip.",
        })
      }
    } catch (err) {
      console.error(err)
  ...
  const handleBulkFetch = async () => {
    if (!formData.anime) {
      return setStatusMessage({
        type: "error",
        text: "Please select an Anime first.",
      })
    }
    ...
    try {
      ...
      setStatusMessage({
        type: "success",
        text: res.data.message || "Bulk fetch complete!",
      })
    } catch (error) {
      setStatusMessage({
        type: "error",
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData.anime || !formData.episodeNumber) {
      return setStatusMessage({
        type: "error",
        text: "Anime selection and episode number are required.",
      })
    }
    ...
    if (activeTab === "file" && !files.video) {
      return setStatusMessage({
        type: "error",
        text: "Video file is required for Local Upload.",
      })
    }
    if (activeTab === "link" && !formData.videoUrl) {
      return setStatusMessage({
        type: "error",
        text: "Direct M3U8 link is required.",
      })
    }
    ...
    try {
      if (activeTab === "link") {
        ...
        setStatusMessage({
          type: "success",
          text: "Episode link saved successfully! It is now live.",
        })
        setFormData((prev) => ({
          ...prev,
          title: "",
        ...
        payload.append(
          "title",
          formData.title || `Episode ${formData.episodeNumber}`,
        )
        payload.append("video", files.video)
        Array.from(files.subtitles).forEach((file) =>
          payload.append("subtitles", file),
        )
        Array.from(files.audios).forEach((file) =>
          payload.append("audios", file),
        )

        if (formData.scheduledAt)
          payload.append("scheduledAt", formData.scheduledAt)
        if (formData.introStart)
          payload.append("introStart", formData.introStart)
      ...
      <div className="mb-8 flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-neutral-400 text-sm">
            Add episodes via Direct Links (0s delay), Bulk Fetch, or Local
            Encode.
          </p>
        </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="space-y-2 md:col-span-1">
            ...
            <CustomSelect
              options={
                animesList?.map((a) => ({ label: a.title, value: a._id })) || []
              }
            ...
            >
              <div className="bg-green-500/10 border border-green-500/20 rounded-2xl p-6 text-center space-y-3">
                ...
                <h3 className="text-lg font-bold text-white">
                  Bulk Fetch All Episodes
                </h3>
                <p className="text-sm text-neutral-400 max-w-lg mx-auto">
                  Clicking the button below will automatically search the
                  Consumet API for the selected Anime, find all available
                  episodes, extract their streaming links, and instantly save
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-[10px] text-neutral-500 uppercase">
                          {files.subtitles.length} selected
                        </p>
                      </div>
                    </div>
                    <input
                      ...
                      accept=".vtt,.srt"
                      onChange={(e) =>
                        setFiles({ ...files, subtitles: e.target.files })
                      }
                  ...
                  <div className="relative flex items-center justify-between p-4 bg-neutral-950/50 border border-neutral-800 rounded-xl hover:border-neutral-700 transition-colors">
                    ...
                    <input
                      ...
                      accept="audio/*"
                      onChange={(e) =>
                        setFiles({ ...files, audios: e.target.files })
                      }
                      className="absolute inset-0 opacity-0 cursor-pointer"
                    />
                  </div>
          ...
          <div className="bg-neutral-950/30 border border-neutral-800/50 p-5 rounded-2xl">
            <div className="flex items-center justify-between mb-4">
              <h4 className="text-xs font-bold text-neutral-400 uppercase tracking-wider flex items-center gap-2">
                <Clock className="w-4 h-4 text-neutral-500" /> Skip Timestamps
                (Seconds)
              </h4>
              <button
                type="button"
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              {["introStart", "introEnd", "outroStart", "outroEnd"].map(
                (field) => (
        ...
        {statusMessage.text && (
          <motion.div
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            className={`p-4 rounded-xl flex items-center gap-3 text-sm font-medium border ${statusMessage.type === "success" ? "bg-emerald-500/10 border-emerald-500/20 text-emerald-500" : statusMessage.type === "error" ? "bg-red-500/10 border-red-500/20 text-red-500" : "bg-blue-500/10 border-blue-500/20 text-blue-400"}`}
          >

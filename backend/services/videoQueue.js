...
const worker = new Worker(
  ...
  async (job) => {
    ...
    console.log(
      `🚀 [Queue Worker] Locked onto Job ${job.id} for Episode: ${episodeId}`,
    )
    ...
    try {
      ...
      const results = await processAnimeVideo(
        null,
        inputPath,
        [],
        [],
      ...
      if (fs.existsSync(inputPath)) {
        ...
        console.log(
          `🧹 Temp workspace file successfully destroyed: ${inputPath}`,
        )
      }
      ...
      const populatedEpisode = await Episode.findById(episodeId).populate(
        "anime",
        "title",
      )
      ...
      console.error(
        `💥 Background worker thread failure for job ${job.id}:`,
        error,
      )
      await Episode.findByIdAndUpdate(episodeId, {
        status: "failed",

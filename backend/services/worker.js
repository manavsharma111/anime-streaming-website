...
const worker = new Worker(
  "videoProcessing",
  async (job) => {
    const { episodeId, inputPath, audioPaths, subtitlePaths, outputDir } = job.data;
    console.log(
    ...
    try {
      ...
      const result = await processAnimeVideo(null, inputPath, audioPaths || [], subtitlePaths || [], outputDir, job);
      ...
      if (publicUrl) {
        ...
        if (uploadSuccess) {
          console.log(`[Worker] Upload to R2 successful. Deleting local files...`);

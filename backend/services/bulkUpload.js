...
const connection = new Redis(process.env.REDIS_URI || "redis://127.0.0.1:6379", { maxRetriesPerRequest: null });
...
const runBulkUpload = async () => {
  try {
    ...
    try {
      ...
      console.log("Created bulk_drops directory. Please place your files there.");
      process.exit(0);
    }
    ...
    if (videoFiles.length === 0) {
      console.log("No video files found in bulk_drops/. Please add files and run again.");
      process.exit(0);
    }
    ...
    for (const file of videoFiles) {
      ...
      if (!match) {
        console.error(`Skipping ${file} - Invalid naming format. Use: AnimeName_E(Number)_Title.mkv`);
        continue;
      }
      ...
      let animeDoc = await Anime.findOne({ title: { $regex: new RegExp(`^${animeName}$`, "i") } });
      
      if (!animeDoc) {
        console.error(`Skipping ${file} - Anime "${animeName}" not found in Database. Please create the Anime first.`);
        continue;
      }
      ...
      const jsonSidecarPath = path.join(BULK_DIR, file.replace(/\.(mp4|mkv)$/i, '.json'));
      try {
        ...
        console.log(`Loaded skip timings from sidecar JSON for Ep ${episodeNumber}`);
      } catch (err) {
      ...
      const outputDir = path.join(__dirname, "uploads", "processed", newEpisode._id.toString());
      ...
      console.log(`✅ Queued: ${animeDoc.title} - Episode ${episodeNumber} (${title})`);
    }
